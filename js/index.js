...
function chordrootChg() {
  ...
  document.getElementById("sharpOrFlat").value = 1;else

  document.getElementById("sharpOrFlat").value = 2;

  if (index == 2 || index == 7)
  document.getElementById("sharpOrFlat").disabled = false;else
...
function getnotes(canvasname, canvasWidth, canvasHeight, rootIndex, scaleIndex) {
  ...
  if (rootValue == 2 || rootValue == 4 || rootValue == 7 || rootValue == 9 || rootValue == 11) {
    ...
    rootLabel = rootLabel.substring(0, 2);else
    if (Number(sf.options[sf.selectedIndex].value) == 2)
  ...
  var sharporflat;
  if (Number(sf.options[sf.selectedIndex].value) == 1) sharporflat = "sharp";else
  sharporflat = "flat";

  var noteordegree;
  var ng = document.getElementById("nameOrDegree");
  if (Number(ng.options[ng.selectedIndex].value) == 1) noteordegree = "sharp";else
  noteordegree = "degree";

  // Get root row
  //alert("rootLabel - " + rootLabel);
  if (rootLabel.length == 1) column = 1;else
  {
    if (sharporflat == "sharp") column = 2;else
    column = 0;
  }
  ...
  for (var string = 0; string < 6; string++) {
    ...
    for (var fret = 0; fret < 16; fret++) {
        ...
        if (scaledegree == currentfret) {
          for (b = 0; b < 4; b++) {
            ...
            if (NoteValue[noterow][b] == scaledegree) {
              ...
              if (noteordegree == "degree") notelabel = degreestr;else
      ...
      if (foundnote != 0 && showString == '1') {
        ...
        if (noterow == rootrow)
        drawnotes(canvasname, x + fw / 2 + fret * foffset, y + string * soffset, true, width);else
